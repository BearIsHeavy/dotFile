...
-- Ctrl+S: Save file in Neovide with GUI menu support confirmation
-- (General Ctrl+S is already mapped above)
if vim.g.neovide then
  -- Neovide can show native save confirmation dialog
...
-- Show diagnostic float on current line (quick error info)
keymap.set("n", "<leader>de", "<cmd>lua vim.diagnostic.open_float()<CR>", { desc = "Show diagnostic float" })

-- Show diagnostic float on cursor hold (automatic, disabled by default)
...
-- Toggle diagnostic float on cursor hold
-- Helper function and variable defined at module level
...
local function toggle_diagnostic_float()
  ...
  if diagnostic_float_enabled then
    ...
    vim.notify("Diagnostic float enabled", vim.log.levels.INFO)
  else
    pcall(vim.api.nvim_del_augroup_by_name, "DiagnosticFloat")
    vim.notify("Diagnostic float disabled", vim.log.levels.INFO)
  end
end

keymap.set("n", "<leader>dt", toggle_diagnostic_float, { desc = "Toggle diagnostic float on cursor hold" })


-- =========================================================
-- Multi-line Comment Shortcut (Shift+/ or Ctrl+/)
-- =========================================================
...
-- This works in Neovide and most GUI terminals
keymap.set("v", "<C-/>", toggle_linewise_comment, { desc = "Toggle comment for selected lines" })
keymap.set("v", "<C-_>", toggle_linewise_comment, { desc = "Toggle comment for selected lines" })
