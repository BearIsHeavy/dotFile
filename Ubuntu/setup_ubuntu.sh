...
if [[ "$ans" =~ ^[Yy]$ ]] || [[ -z "$ans" ]]; then
    bash "$SCRIPT_DIR/requirements_ubuntu.sh"
fi
...
if [[ "$ans" =~ ^[Yy]$ ]]; then
    bash "$SCRIPT_DIR/initial/vpn.sh" || { echo -e "${RED}VPN setup failed (exit code: $?)${RESET}"; }
fi
...
if [[ "$ans" =~ ^[Yy]$ ]]; then
    bash "$SCRIPT_DIR/initial/zsh.sh" || { echo -e "${RED}Zsh setup failed (exit code: $?)${RESET}"; }
fi
...
if [[ "$ans" =~ ^[Yy]$ ]]; then
    bash "$SCRIPT_DIR/initial/vim.sh" || { echo -e "${RED}Vim setup failed (exit code: $?)${RESET}"; }
fi
...
if [[ "$ans" =~ ^[Yy]$ ]]; then
    bash "$SCRIPT_DIR/initial/nvim.sh" || { echo -e "${RED}Neovim setup failed (exit code: $?)${RESET}"; }
fi
...
if [[ "$ans" =~ ^[Yy]$ ]]; then
    bash "$SCRIPT_DIR/initial/otherCommand.sh" || { echo -e "${RED}Additional tools setup failed (exit code: $?)${RESET}"; }
fi
...
if [[ "$ans" =~ ^[Yy]$ ]]; then
    bash "$SCRIPT_DIR/initial/terminal_fonts.sh" || { echo -e "${RED}Font setup failed (exit code: $?)${RESET}"; }
fi
